...
the data have been collected in class, you may use the data from
previous years, stored in files `laptop_2019.csv` and
`laptop_2020.csv`. To use these data, rename one of them as
`laptops.csv`.
...
```{r laptops-subset-4, echo=FALSE}
subset(laptops, os == "windows")
```
...
operations can be combined together, for example,
...
answer against R's built-in solution, `y <- sd(x)`. *Hint:* Use the
...
In-class activity: what is a "factor"?
======================================
...
not as a native data type. Here we will explore factors and understand
how and when they could be useful.

None of the columns in our `laptops` table are a factor, but we can
create a factor from one of the columns using a function appropriately
...
Let's now inspect the contents of `x` and `y`:
...
How should we interpret the second data summary?
...
Choosing the right representation of your data can be important to
your analysis. Judicious use of factors can also help you perform
complex calculations very simply. (In fact you may find that factors
are useful in the programming challenge.)
...
Use this code to create a *script* or a *program*---a text file
containing the code necessary to run the data analysis, starting with
reading the data into R, and ending with the calculations that
produce the answers.
...
such as `analyze_laptops_data.R`. Also, a few comments (these are
lines starting with "`#`") explaining in plain language what the code
does. We will use Etherpad to share our programs.
...
[`jnovembre.github.io/BSD-QBio7`][bsd-qbio-site] and follow the
...
[bsd-qbio-site]: https://jnovembre.github.io/BSD-QBio7
