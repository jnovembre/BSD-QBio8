...
Chicago's Divvy bike-share system (the light blue bikes you will see around town) collects data on all its rides and shares them publically, after anonymizing the rider info (https://www.divvybikes.com/system-data).  The file `data/202107-divvy-tripdata.csv` contains a list of all the Divvy bike rides taken in Chicago in July 2021. Form small groups and work on the following exercises.  Hint: use the package `lubridate` to work with days, dates, time:

- **Ride map** write a function that takes as input a calendar date (YYYY-MM-DD), and draws a map of all the starting points of rides. Mark a point for each occurrence using the starting latitude and longitude `start_lat` and `start_lng`. Set the `alpha` to something like 0.1 to show brighter colors in areas with many occurrences.  Use colour to indicate (Optional: Add a feature that draws a line from the starting location to end location)
...
- **Miscellaneous**: if you'd like to play more, the `spDistsN1` function of the sp library returns distances as a function of latitute/longitude pairs.  
